    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        ...
        if (request.method === "GET" && url.pathname.startsWith("/images/")) {
          const requestedFile = basename(decodeURIComponent(url.pathname.slice("/images/".length)));
          ...
          try {
            const contents = await readFile(join(imagesDir, requestedFile));
            response.statusCode = 200;
            response.setHeader("Content-Type", mimeTypeForFile(requestedFile));
...
function mimeTypeForFile(fileName: string) {
  return mimeTypeByExtension[extname(fileName).toLowerCase()] ?? "application/octet-stream";
}
