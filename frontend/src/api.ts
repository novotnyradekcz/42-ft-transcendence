...
export async function listFriends(): Promise<UserProfile[]> {
  ...
  try {
    ...
    const users = normalizeUsers(
      await requestJson<unknown>(`/friends/show?${params.toString()}`),
    );
    return users.map(applyStoredProfile);
  } catch {
    return [];
  }
}
...
export async function addFriend(userId: number): Promise<UserProfile[]> {
  ...
  await requestJson<unknown>("/friends/add", {
    method: "POST",
...
export async function removeFriend(userId: number): Promise<UserProfile[]> {
  ...
  await requestJson<unknown>("/friends/remove", {
    method: "POST",
    body: JSON.stringify({ userId: currentUser.id, friendId: userId }),
  });
  return listFriends();
