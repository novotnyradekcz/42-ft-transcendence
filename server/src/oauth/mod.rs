...
pub async fn oauth_callback(
    ...
    let db_user = {
        ...
        match find_or_create_oauth_user(&mut db, &profile, &pool.encoder) {
            Ok(u) => u,
            Err(OAuthUserError::NoEmail) => {
                log::warn!("{provider_id} login for {} carried no email", profile.login);
...
fn parse_profile(provider: &OAuthProvider, raw: &serde_json::Value) -> Option<OAuthProfile> {
    ...
    // the id is no longer stored, but it still has to be there: a response
    // without one isn't a profile we understand, and it's the login fallback
